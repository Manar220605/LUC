...
export default function ForgotEmailPage() {
  ...
  return (
    <main className="mx-auto max-w-lg px-4 py-12">
      <h1 className="text-3xl font-bold text-lu-deep">Forgot email</h1>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-4">
        <div>
          <label htmlFor="fileNumber" className="block text-sm font-medium text-ink">
            Student file number
          </label>
          <input
            id="fileNumber"
        ...
        <button
          type="submit"
          disabled={submitting}
          className="rounded-md bg-lu px-4 py-2 text-sm font-medium text-white hover:bg-lu-dark disabled:opacity-50"
        >
      ...
      {result && (
        <div className="mt-8 rounded-lg border border-lu/10 bg-lu-mist p-4">
          <h2 className="text-lg font-semibold text-lu-deep">Recovering your email</h2>
          <p className="mt-2 text-sm text-muted">{result.message}</p>
          {result.maskedEmail && (
            <p className="mt-4 text-center text-xl font-medium text-lu-dark">{result.maskedEmail}</p>
          )}
        </div>
      )}
