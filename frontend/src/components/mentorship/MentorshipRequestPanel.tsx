...
export default function MentorshipRequestPanel({ alumniUserId, alumniName }: Props) {
  ...
  if (sessionStatus === 'loading' || loading) {
    return (
      <section id="mentorship" className="mt-6 rounded-lg border border-lu/10 bg-white p-4">
        <p className="text-sm text-muted">Loading mentorship…</p>
      </section>
    );
  }
  ...
  if (sessionStatus !== 'authenticated') {
    return (
      <section id="mentorship" className="mt-6 rounded-lg border border-lu/10 bg-white p-4">
        <h2 className="text-sm font-semibold text-lu-deep">Mentorship</h2>
      ...
      {status?.canRequest && (
        <form onSubmit={handleSubmit} className="mt-3">
          <label className="block text-sm">
            <span className="font-medium text-ink">Short message</span>
            <textarea
              value={message}
          ...
          <p className="mt-1 text-xs text-muted">{message.length}/500</p>
          <button
            type="submit"
            disabled={submitting}
            className="mt-3 rounded-md bg-lu px-4 py-2 text-sm font-medium text-white hover:bg-lu-dark disabled:opacity-50"
          >
